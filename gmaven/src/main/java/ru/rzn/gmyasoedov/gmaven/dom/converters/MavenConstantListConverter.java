...
public abstract class MavenConstantListConverter extends ResolvingConverter<String> {
  ...
  public String getErrorMessage(@Nullable String s, ConvertContext context) {
    return new HtmlBuilder()
      .append(GDomBundle.message("specified.value.is.not.acceptable.here"))
      .append(HtmlChunk.br())
      .append(GDomBundle.message("acceptable.values"))
      .appendWithSeparators(HtmlChunk.text(", "),
                            ContainerUtil.map(getValues(context), HtmlChunk::text))
      .toString();
  }
}